...
// function isAdMode(frm) {
//     return frm.doc.is_ad === 1;
// }

// function toggleAdFields(frm) {
//     const adFields = [
//         'campagin',
//         'select_ad_account',
//         'select_ad_set',
...
//         if (!validateAdBeforePublish(frm)) {
//             return;
//         }
...
//         // ✅ 1. If valid scheduled_time already exists → schedule directly
//         if (existing && existing.isAfter(now)) {
//             frappe.call({
//                 method: 'frappe_social.frappe_social.api.posts.schedule',
//                 args: {
...
//                         frappe.msgprint({
//                             message: __(
//                                 'Post has been scheduled and will be published on {0}.',
//                                 [timeStr]
//                             ),
//                             indicator: 'blue'
//                         });
...
//         // ❌ 2. Only ask if scheduled_time is missing or invalid
//         show_new_time_prompt();
...
//                 if (selected.isBefore(now)) {
//                     frappe.show_alert({
//                         message: __('Cannot schedule for a past time.'),
//                         indicator: 'red'
//                     }, 5);
//                     return;
//                 }
...
//                             const timeStr = frappe.datetime.str_to_user(values.scheduled_time);
//                             frappe.msgprint({
//                                 message: __('Post has been scheduled for {0}.', [timeStr]),
//                                 indicator: 'blue'
//                             });
//                         } else {
//                             frappe.msgprint({
...
//         frm.set_value('campagin', '');
//         frm.set_value('select_ad_account', '');
//         frm.set_value('select_ad_set', '');
//         frm.set_value('selected_facebook_page', '');
...
//         if (frm.doc.platform && frm.doc.organization) {
//             setup_ad_filters(frm);
//         }
...
//         if (frm.doc.campagin) {
//             // Fetch campaign details and auto-populate ad account
//             frappe.call({
...
//                 callback: function (r) {
//                     if (r.message) {
//                         if (!r.message.custom_is_meta_ads) {
//                             frappe.msgprint({
//                                 title: __('Invalid Campaign'),
//                                 indicator: 'red',
//                                 message: __('Selected campaign is not a Meta Ads campaign')
//                             });
//                             frm.set_value('campagin', '');
//                             return;
//                         }
...
//                                 title: __('Warning'),
//                                 indicator: 'orange',
//                                 message: __('This campaign has no associated ad account')
//                             });
//                         }
//                     }
//                 }
...
//         if (frm.doc.select_ad_account) {
//             // Fetch and populate Facebook pages
//             frappe.call({
//                 method: 'frappe.client.get',
//                 args: {
//                     doctype: 'Ads Account Integration',
//                     name: frm.doc.select_ad_account,
//                     fields: ['fb_pages', 'platform', 'connection_status']
//                 },
//                 callback: function (r) {
//                     if (r.message) {
//                         // Check connection status
//                         if (r.message.connection_status !== 'Connected') {
//                             frappe.msgprint({
//                                 title: __('Account Not Connected'),
//                                 indicator: 'orange',
//                                 message: __('This ad account is not connected. Please reconnect it.')
//                             });
//                         }

//                         // Populate Facebook pages
//                         if (r.message.fb_pages && r.message.fb_pages.length > 0) {
//                             // Store pages data in form for later use
//                             frm._fb_pages_data = r.message.fb_pages;

//                             // Show page selector dialog
//                             show_page_selector_dialog(frm, r.message.fb_pages);

//                             frappe.show_alert({
//                                 message: __(`${r.message.fb_pages.length} Facebook page(s) available`),
//                                 indicator: 'green'
//                             });
//                         } else {
//                             frappe.msgprint({
...
//                                 indicator: 'orange',
//                                 message: __('No Facebook pages found for this ad account.')
//                             });
//                         }
//                     }
//                 }
...
//                     fields: ['adset_id', 'campaign', 'enable_ad_set']
//                 },
//                 callback: function (r) {
//                     if (r.message) {
//                         if (!r.message.adset_id) {
//                             frappe.msgprint({
//                                 title: __('Warning'),
//                                 indicator: 'orange',
//                                 message: __('This ad set has not been created on Meta yet')
//                             });
//                         }

//                         // Verify campaign matches
//                         if (r.message.campaign !== frm.doc.campagin) {
//                             frappe.msgprint({
//                                 title: __('Mismatch'),
//                                 indicator: 'red',
//                                 message: __('Ad Set belongs to a different campaign')
//                             });
//                         }
//                     }
//                 }
...
// function setup_ad_filters(frm) {
//     // Filter campaigns: only Meta Ads campaigns for the selected organization
//     frm.set_query('campagin', function () {
//         let filters = {
//             'custom_is_meta_ads': 1
//         };

//         if (frm.doc.organization) {
//             filters['organisation'] = frm.doc.organization;
//         }

//         return { filters: filters };
//     });

//     // Filter ad accounts: only accounts matching the platform and organization
//     frm.set_query('select_ad_account', function () {
//         let filters = {};

//         if (frm.doc.platform) {
//             filters['platform'] = frm.doc.platform;
//         }
...
//     // Filter ad sets: only ad sets for the selected campaign
//     frm.set_query('select_ad_set', function () {
//         if (!frm.doc.campagin) {
//             frappe.msgprint(__('Please select a campaign first'));
//             return { filters: { 'name': ['=', ''] } };
//         }

//         return {
//             filters: {
//                 'campaign': frm.doc.campagin
//             }
//         };
//     });
// }
...
function toggleAdFields(frm) {
    // Campaign field is available for both ads and normal posts
    const adOnlyFields = [
        'select_ad_account',
        'select_ad_set',
        'selected_facebook_page'
...
frappe.ui.form.on('Social Post', {
    ...
    refresh: function (frm) {
        ...
        if (frm.is_new()) {
            ...
            $('.page-actions button.btn.btn-default.ellipsis').show();
            $('.btn-secondary[data-label="Cancel"]').remove();
            if (document.querySelector('.menu-btn-group')) {
                document.querySelector('.menu-btn-group').style.display = 'none';
            }
            $('.page-actions button.text-muted.btn.btn-default.icon-btn').hide();
            $('.page-actions button.text-muted.btn.btn-default.prev-doc').show();
        ...
        // Display IDs if created
        if (frm.doc.ad_id) {
            frm.set_df_property('ad_id', 'description',
                `✓ Ad created on Meta: ${frm.doc.ad_id}`);
        }
        if (frm.doc.post_id) {
            frm.set_df_property('post_id', 'description',
                `✓ Post published: ${frm.doc.post_id}`);
        }
    ...
    // Check if this is an ad
        if (isAdMode(frm)) {
            // For ads, use the publish_ad method
            if (!validateAdBeforePublish(frm)) {
                return;
            }
            ...
            frappe.confirm(
                ...
                function () {
                    frappe.call({
                        method: 'frappe_social.frappe_social.doctype.social_post.social_post.publish_ad',
                        args: {
                            post_name: frm.doc.name
                        },
                        freeze: true,
                        freeze_message: __('Publishing ad...'),
                        callback: function (r) {
                            if (r.message) {
                                if (r.message.success) {
                                    frappe.msgprint({
                                        message: __('Ad published successfully!'),
                                        indicator: 'green'
                                    });
                                    frm.reload_doc();
                                } else {
                                    frappe.msgprint({
                                        message: __('Ad publishing failed: ') + r.message.error_message,
                                        indicator: 'red'
                                    });
                                }
                            }
        ...
        // For normal posts, use existing logic
        if (!validateAdBeforePublish(frm)) {
            return;
        }
    ...
    campagin: function (frm) {
        ...
        if (frm.doc.campagin) {
            ...
            frappe.call({
                ...
                callback: function (r) {
                    if (r.message) {
                        // Only auto-populate ad account if in ad mode
                        if (isAdMode(frm)) {
                            if (!r.message.custom_is_meta_ads) {
                                frappe.msgprint({
                                    title: __('Invalid Campaign'),
                        ...
                        // For normal posts, campaign is just for tracking/UTM
                    }
                }
            });
        }

        // Rebuild UTM if link exists
        if (frm.doc.link) {
            auto_build_utm(frm);
        }
    ...
    select_ad_account: function (frm) {
        ...
        if (frm.doc.select_ad_account) {
            ...
            frappe.call({
                ...
                args: {
                    doctype: 'Ads Account Integration',
                    filters: { name: frm.doc.select_ad_account },
                    fieldname: ['connection_status']
                },
                callback: function (r) {
                    if (r.message && r.message.connection_status !== 'Connected') {
                        frappe.msgprint({
                            title: __('Account Not Connected'),
                            indicator: 'orange',
                            message: __('This ad account is not connected. Please reconnect it.')
    ...
    select_ad_set: function (frm) {
        // Clear Facebook page first
        frm.set_value('selected_facebook_page', '');

        if (frm.doc.select_ad_set) {
            // Fetch Ad Set details including Facebook page
            frappe.call({
                method: 'frappe.client.get',
                args: {
                    doctype: 'Ad Set',
                    name: frm.doc.select_ad_set,
                    fields: ['adset_id', 'campaign', 'enable_ad_set', 'select_facebook_page']
                },
                callback: function (r) {
                    if (r.message) {
                        // Check if ad set has been created on Meta
                        if (!r.message.adset_id) {
                            frappe.msgprint({
                                title: __('Warning'),
                                indicator: 'orange',
                        ...
                        // Auto-populate Facebook page from Ad Set
                        if (r.message.select_facebook_page) {
                            frm.set_value('selected_facebook_page', r.message.select_facebook_page);
                            
                            frappe.show_alert({
                                message: __(`Auto-filled Facebook Page: ${r.message.select_facebook_page}`),
                                indicator: 'green'
                            }, 3);
                        } else {
                            frappe.msgprint({
                                title: __('No Facebook Page'),
                                indicator: 'orange',
                                message: __('This ad set does not have a Facebook page selected. Please select one in the Ad Set.')
                            });
                        }
                    }
                }
...
function setup_campaign_filter(frm) {
    frm.set_query('campagin', function () {
        let filters = {};

        if (isAdMode(frm)) {
            // For ad mode, show only Meta Ads campaigns (custom_is_meta_ads = 1)
            filters['custom_is_meta_ads'] = 1;
        } else {
            // For normal posts (is_post, is_reel, is_story), show only non-Meta Ads campaigns
            if (frm.doc.is_post || frm.doc.is_reel || frm.doc.is_story) {
                filters['custom_is_meta_ads'] = 0;
            }
        }
...
function setup_ad_filters(frm) {
    ...
    frm.set_query('select_ad_account', function () {
        ...
        if (frm.doc.organization) {
            // Changed from 'organization' to 'organisation' (with 's')
            filters['organisation'] = frm.doc.organization;
        }
...
function update_character_count(frm) {
    ...
    function applyColors() {
        ...
        let textColor;
        if (isExceeded) {
            textColor = isDark ? '#ff6b6b' : '#c62828';
        } else {
        ...
        const html_content = `
            <div style="display: flex; justify-content: flex-end; align-items: center;">
                <div style="display: flex; align-items: center; color: ${textColor}; font-weight: 400; font-size: 12px;">
                    <span>${message}</span>
                </div>
            </div>
        `;
...
function filter_account_field(frm) {
    ...
    if (!organization || !platform || isAdMode(frm)) {
        frm.set_query('account', () => ({ filters: { name: ["in", []] } }));
        if (isAdMode(frm)) {
            frm.set_value('account', '');
        }
...
function update_account_details(frm) {
    ...
    async function renderHTML() {
        ...
        if (!account || isAdMode(frm)) {
            html_content = `
                <div style="padding: 10px; border: 1px dashed ${colors.placeholderBorder}; border-radius: 8px; 
                            color: ${colors.placeholderText}; font-style: italic; text-align: center; font-size: 14px;">
                    ${isAdMode(frm) ? 'Ad mode - Use Ad Account settings' : 'Select an Organization → Platform → Account to see details here'}
                </div>`;
        } else {
            try {
                const response = await frappe.db.get_doc('Social Integration', account);
                ...
                html_content = `
                    <div>
                        ...
                        <div style="padding: 20px; border-radius: 12px; background-color: ${colors.containerBg}; 
                                    box-shadow: ${colors.containerShadow}; border: 1px solid ${colors.containerBorder}; margin: 10px 0;">
                            <div style="display: flex; align-items: flex-start; gap: 40px; flex-wrap: wrap;">
                                ${img_html}

                                <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px 50px; flex: 1; 
                                            font-size: 14px; line-height: 1.8; min-width: 600px;">
                                    <div style="margin-top: 10px;">
                                        <div style="margin-bottom: 12px;">
                                            <strong>Organization :</strong>
                                            <span style="color: ${colors.textSecondary}; margin-left: 8px;">${org}</span>
                                    ...
                                    <div style="display: flex; align-items: flex-start; margin-top: 10px;">
                                        <strong style="flex-shrink: 0;">Description :</strong>
                                        <span style="color: ${colors.textSecondary}; margin-left: 8px; word-break: break-word; 
...
frappe.ui.form.on('Social Post Media', {
    file: function (frm, cdt, cdn) {
        ...
        frappe.db.get_value(
            ...
            (r) => {
                if (r) {
                    frappe.model.set_value(cdt, cdn, 'file_size', r.file_size);
                    frappe.model.set_value(cdt, cdn, 'file_type', r.file_type);
                }
            }
        );
    }
